...
public class AIPath : MonoBehaviour {
	...
	{
		if (col.gameObject.name.StartsWith ("Human")) {
						target = null;
				}
		}
	...
	public IEnumerator RepeatTrySearchPath () {
		GameObject danger_zone = GameObject.Find ("Danger");
		float distance;

		//sets the target to the human with the smallest distance to the danger area, but only if vector of human points toward it)
		while (true) {
								
			float smallestdistance = 100; int i = 0;
			RaycastHit[] hits;			
			foreach (var human in Spawn.humans) {

							//distance = Mathf.Abs(Vector3.Distance (danger_zone.transform.position, human.transform.position));
							
					if (human!=null && target==null) {
					hits = Physics.RaycastAll(human.transform.position, human.transform.forward);
					//human.transform.eulerAngles

					foreach (var item in hits) {
						distance = item.distance;			
						if (distance < smallestdistance && item.collider.gameObject.name.StartsWith("Danger"))
						{
							if (smallestdistance!=100)
							{
								Debug.Log ("Target changed!!!!!!");
							}
							Debug.Log ("Target aquired, Distance: " + distance);
